...
class MillRelineBuilder(ProjectBuilder):
    ...
    def _build_stripping_fh(self, inputs: RelineScope, start_dt: dt.datetime) -> Phase:
        ...
        remaining_fh = inputs.feed_end.n_liner_strip
        #fh_sequence = (remaining_fh - 3 * i for i in range(remaining_fh, start=1))

        remaining_filler = inputs.feed_end.n_filler_strip
        ...
        for i in range(inputs.shell.n_rows_strip // 2):
            # ---------------------------------
            # Stripping Task
            # ---------------------------------
            ...
            if i == inputs.shell.n_rows_strip / 2 - 1:
                continue
    ...
    def _build_install_fh_shell(self, inputs: RelineScope, start_dt: dt.datetime) -> Phase:
        ...
        for i in range(inputs.shell.n_rows_install // 2):
            # ---------------------------------
            # Install Task
            # ---------------------------------
            ...
            # do not inch on the last row of shell
            if i == inputs.shell.n_rows_install / 2 - 1:
                continue
    ...
    def _build_strip_discharge_grates_pulps_fillers(self, inputs: RelineScope, start_dt: dt.datetime) -> Phase:
        ...
        num_iterations = inputs.discharge.n_grates_strip - 1
        ...
        for i, work_task in enumerate(it):
            ...
            duration = inputs.discharge.t_grate_strip + inputs.discharge.t_pulp_strip
            ...
            # do not inch on the last row of shell
            # CHECK
            if i == inputs.shell.n_rows_install - 2:
                continue
    ...
    def _build_install_pulp_lifters(self, inputs: RelineScope, start_dt: dt.datetime) -> Phase:
        ...
        task_start = start_dt
        prev_task = None
        for i in range(inputs.discharge.n_pulps_install):
            duration = inputs.discharge.t_pulp_install
            task_end = task_start + dt.timedelta(minutes=duration)
    ...
    def _build_install_de_grates_fillers(self, inputs: RelineScope, start_dt: dt.datetime) -> Phase:
        ...
        for i in range(inputs.discharge.n_grates_install):
            duration = inputs.discharge.t_grate_install
            task_end = task_start + dt.timedelta(minutes=duration)
    ...
    def build(self, inputs: RelineScope) -> Project:
        project = Project(
            ...
            project_type=ProjectType.MILL_RELINE
        )
        ...
        if inputs.discharge.replace_dc:      
            project.add_phase(
        ...
        # ------------------------------------------
        if inputs.discharge.replace_dc:
            t_discharge = inputs.discharge.t_install_dc
