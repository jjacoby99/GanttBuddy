...
def inject_todo_panel_css() -> None:
    st.markdown(f"<style>{_todo_panel_css()}</style>", unsafe_allow_html=True)
...
def _todo_dataframe(items: list[dict[str, Any]], timezone: ZoneInfo, valid_project_ids: set[str]) -> pd.DataFrame:
    rows: list[dict[str, Any]] = []
...
def _render_todo_overview_panel(todos_df: pd.DataFrame, project_name_by_id: dict[str, str]) -> None:
    if todos_df.empty:
    ...
    st.caption("Your todo signal for projects in this organization, with the hottest follow-ups surfaced first.")
    st.markdown(
        f"""
        ...
        if status_chart_df.empty:
            ...
            chart = (
                ...
                .encode(
                    ...
                    color=alt.Color("status:N", title=None, scale=alt.Scale(domain=status_chart_df["status"].tolist(), range=status_chart_df["color"].tolist())),
    ...
    with queue_col:
        ...
        if high_priority_df.empty:
            ...
            for _, row in queue.iterrows():
                due_label = row["due_dt"].strftime("%b %d") if row["due_dt"] is not None and not pd.isna(row["due_dt"]) else ""
                st.markdown(
                    f"""
